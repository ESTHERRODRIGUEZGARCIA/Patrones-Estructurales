import sys
import csv
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QPushButton, QLabel, QInputDialog, QDialog, QFormLayout, QComboBox
import uuid
...
class ComboElementsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        for combo_category, options in combo_options.items():
            ...
            if combo_category in ['Entrante', 'PizzaMenu', 'Bebida', 'Postre']:
                combo_box = QComboBox(self)
                combo_box.addItem("Selecciona...")
                for option in options:
                    combo_box.addItem(f"{option['name']} - {', '.join(option['ingredients'])} - {option['price']} euros", option)
                combo_box.currentIndexChanged.connect(lambda _, cat=combo_category, box=combo_box: self.select_combo_item(cat, box.currentData()))
